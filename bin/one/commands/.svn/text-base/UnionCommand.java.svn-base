...
/**
 * Union two query lists.
 */
public class UnionCommand implements ICommand, IQueryList {

	private Pattern pattern = Pattern.compile("union(.*)and(.*);", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
	...
	public void execute() throws ChrisException {
		...
		if (a.compareTo(b) != -1){
			name = "union_"+a.getName()+"_"+b.getName();
			Table temp = new Table(name);
			temp.addFields(a.getFieldNames());
			temp.addDataset(a.unionTable(b));
